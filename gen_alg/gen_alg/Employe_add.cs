...
namespace gen_alg
{
    public partial class Employe_add : Form
    {
        string work_file_name;

        int id = -1;    //идентификатор изменяемого или добавляемого сотрудника

        Employee adding_empl;    //добавляемый/изменяемый сотрудник
        Dictionary<string, Dictionary<int, string>> attributes_list;   //массив атрибутов и всех их значений 
        List<string> required_attr; //массив обязательных атрибутов сотрудника
        Dictionary<List<ComboBox>, NumericUpDown> attributes;   //массив пар элементов: выпадающий списков и числовых полей
        //кадый элемент массива содержит массив из пары выпадающих списков, первый из которых - список атрибутов, второй - список значений, числовое поле - вес атрибута для сотрудника
       
        XmlDocument empl_xml_doc;   //документ employee.xml


        List<string> required_attrs_value;  //массив значений обязательных атрибутов
        Dictionary<KeyValuePair<string, int>, string> attributes_values;    //массив атрибутов сотрудника
        ...
        public Employe_add(string work_file_name)
        {
        ...
        public Employe_add(string work_file_name, int id)
        {
            this.work_file_name = work_file_name;
            this.id = id;
        ...
        private void Employe_add_Load(object sender, EventArgs e)   //загрузка окна
        {
            this.CenterToParent();

          
            adding_empl = new Employee();
            ...
            attributes_list = Read_XML_employees(attributes_list);

            //если нужно изменить информацию о сотруднике, то отображаем информацию о сотруднике
            if (id >= 0)
            {
            ...
            else //иначе создаём заготовку для создания нового сотрудника
            {
        ...
        private void ComboBox_attr_val_SelectedIndexChanged(object sender, System.EventArgs e)  //изменение значения атрибута с списке
        {
        ...
        private void View_changable_pr()    //метод для отображения изменяемого сотрудника
        {
            //выборка информации о сотруднике
            foreach (Employee proj in Form1.employees)
            {
                if (proj.ID == this.id)
                {
                    adding_empl = proj;
            ...
            dgv_requ_attr.Rows.Add("ФИО", adding_empl.Name);
            dgv_requ_attr[0, 0].ReadOnly = true;
            dgv_requ_attr.Rows.Add("Возраст", adding_empl.Age);
            dgv_requ_attr[0, 1].ReadOnly = true;
            dgv_requ_attr.Rows.Add("Опыт работы", adding_empl.Experience);
            dgv_requ_attr[0, 2].ReadOnly = true;

            //очистка панели дополнительных атрибутов
            Panel_clear();

            //отображение атрибутов сотрудника, их значений и весов
            foreach (KeyValuePair<KeyValuePair<string, int>, string> attr in adding_empl.Get_all_attrib())
            {
        ...
        private void View_maket_for_add_pr()     //создание шаблона для добавления сотрудника
        {
            //очистка таблицы обязательных атрибутов
            Clear_table();
            ...
            dgv_requ_attr.Rows.Add("ФИО", adding_empl.Name);
            dgv_requ_attr[0, 0].ReadOnly = true;
            dgv_requ_attr.Rows.Add("Возраст", adding_empl.Name);
            dgv_requ_attr[0, 1].ReadOnly = true;
            dgv_requ_attr.Rows.Add("Опыт работы", adding_empl.Name);
            dgv_requ_attr[0, 2].ReadOnly = true;
        }

        private void Write_new_attr_in_XML(string attr) //запись атрибута в файл с информацией об атрибутах сотрудников
        {
            //создание нового элемента и запись его в файл
            empl_xml_doc.Load("employees.xml");

            XmlElement root = empl_xml_doc.DocumentElement;

            XmlNode attr_list = root.LastChild;

            XmlElement new_attr = empl_xml_doc.CreateElement("attr");
            XmlAttribute name_attr = empl_xml_doc.CreateAttribute("name");
            XmlText text_name = empl_xml_doc.CreateTextNode(attr);

            name_attr.AppendChild(text_name);
            new_attr.Attributes.Append(name_attr);
            attr_list.AppendChild(new_attr);
            empl_xml_doc.Save("employees.xml");
        }

        private void Write_attr_val_in_XML(string attr, Dictionary<int, string> values) //запись информации о значениях атрибута в файл об атрибутах сотрудников
        {
            ...
            foreach (XmlNode attributes in attr_list.ChildNodes)
            {
                ...
                if (name != null)
                {
                    if (name.Value == attr)
                    {
                        ...
                        {
                            XmlElement new_attr = empl_xml_doc.CreateElement("value");

                            XmlAttribute id_attr = empl_xml_doc.CreateAttribute("id");
                            XmlText text_id = empl_xml_doc.CreateTextNode(val.Key.ToString());
                            XmlAttribute name_attr = empl_xml_doc.CreateAttribute("name");
                            XmlText text_name = empl_xml_doc.CreateTextNode(val.Value);
        ...
        public Dictionary<string, Dictionary<int, string>> Read_XML_employees(Dictionary<string, Dictionary<int, string>> attributes_list)  //чтение файла об атрибутах сотрудников
        {
            //чтение происходит в соответствии со структурой файла, описанной в приложении
            empl_xml_doc = new XmlDocument();
            empl_xml_doc.Load("employees.xml");

            foreach (XmlNode root in empl_xml_doc.DocumentElement)
            {
        ...
        {
            //формирование идентификатора сотрудника (при добавлении), занесение информации об атрибутах в массивы обязательных и дополнительных атрибутов сотрудника
            ...
            if (id < 0)
            {
                ...
                foreach (Employee pr in Form1.employees)
                {
                    ids.Add(pr.ID);
                }
